...
static void knob_handler(knob_dev_t *knob)
{
    ...
    // ESP_LOGI(TAG,"pha_value : %d,phb_value: %d",pha_value,phb_value);
    if ((knob->state) > 0) {
        knob->ticks++;
    }
    ...
    // ESP_LOGI(TAG,"knob state: %d",knob->state);
    switch (knob->state) {
    case KNOB_READY:
...
knob_handle_t iot_knob_create(const knob_config_t *config)
{
    ...
    ESP_LOGI("Knob","GPIO A level: %d,GPIO B level: %d", knob->encoder_a_level,knob->encoder_b_level);
