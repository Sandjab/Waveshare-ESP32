...
{
    printf("KONB: KONB_RIGHT,count_value:%d \r\n", iot_knob_get_count_value((knob_handle_t)arg));
    ESP_LOGI(TAG, "KONB: KONB_RIGHT,count_value:%d", iot_knob_get_count_value((knob_handle_t)arg));
    surface_dial_report(DIAL_L);
}

static void _knob_left_cb(void *arg, void *data)
{
    printf("KONB: KONB_LEFT,count_value:%d \r\n", iot_knob_get_count_value((knob_handle_t)arg));
    ESP_LOGI(TAG, "KONB: KONB_LEFT,count_value:%d", iot_knob_get_count_value((knob_handle_t)arg));
...
static void _knob_init(void)
{
    ...
    esp_err_t ret = iot_knob_register_cb(s_knob, KNOB_LEFT, _knob_left_cb, NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG,"iot_knob_register_cb failed");
    }
...
// #endif
    ...
    while (1) {
        tud_task();
        // int value = iot_knob_get_count_value(s_knob);
